...
describe('ElementLocator', () => {
  ...
  describe('#withContext', () => {
    it('should switch to default content and frame when frame >= 0', async () => {
      const callback = sandbox.stub().resolves('result');
      
      mockDriver.switchTo().defaultContent.resolves();
      mockDriver.switchTo().frame.resolves();
      
      const result = await locator['#withContext'](1, callback);
      
      expect(mockDriver.switchTo().defaultContent.calledOnce).toBe(true);
      expect(mockDriver.switchTo().frame.calledOnce).toBe(true);
      expect(callback.calledOnce).toBe(true);
      expect(result).toBe('result');
    });

    it('should not switch frame when frame < 0', async () => {
      const callback = sandbox.stub().resolves('result');
      ...
      expect(mockDriver.switchTo().defaultContent.calledOnce).toBe(true);
      expect(mockDriver.switchTo().frame.called).toBe(false);
      expect(callback.calledOnce).toBe(true);
      expect(result).toBe('result');
    });
    ...
    it('should handle NoSuchFrameError gracefully', async () => {
      ...
      mockDriver.switchTo().defaultContent.resolves();
      mockDriver.switchTo().frame.rejects({ name: 'NoSuchFrameError' });
    ...
    it('should re-throw non-NoSuchFrameError errors', async () => {
      const callback = sandbox.stub().resolves('result');
      
      mockDriver.switchTo().defaultContent.resolves();
      mockDriver.switchTo().frame.rejects({ name: 'OtherError' });
      
      await expect(locator['#withContext'](1, callback)).rejects.toThrow('OtherError');
    });
  });

  describe('findChildElements', () => {
    it('should find child elements with context switching', async () => {
      const parent = { frame: 0 };
      const childData = { id: 'test', type: 'element', exact: false };
      
      const mockElement = { rect: { height: 10, width: 10 } };
      mockDriver.findElements.resolves([mockElement]);
      mockDriver.switchTo().defaultContent.resolves();
      mockDriver.switchTo().frame.resolves();

...
// Check state (use assertions for validations)
...
```javascript
// Conditionals (return boolean for branching logic)
const disabled = await browser.button('Submit').is.disabled()
const enabled = await browser.button('Submit').is.enabled()
const notVisible = await browser.element('Item').is.not.visible()

if (notVisible) {
  // Handle hidden element
}

// Assertions (throw errors on validation failure)
await browser.element('Loading').should.not.be.visible() // Assert element disappears
await browser.button('Ready').should.be.visible() // Assert element is displayed
await browser.button('Submit').should.be.disabled() // Assert element is disabled
await browser.button('Submit').should.be.enabled() // Assert element is enabled
```
...
// Verify modal closed
await browser.dialog('User Details').should.not.be.visible()
```
...
- **Conditionals** (return true/false): `is.disabled()`, `is.enabled()`, `is.not.visible()`, `is.not.disabled()`
- **Assertions** (throw errors): `should.be.visible()`, `should.not.be.visible()`, `should.be.disabled()`, `should.not.be.disabled()`, `should.be.enabled()`
...
7. **Use correct state check** - `is.not.visible()` for conditionals (returns boolean), `should.be.visible()`/`should.not.be.visible()` for assertions (throw errors)

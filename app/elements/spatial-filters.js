...
/**
 * Filters elements to find those within another element's subtree.
 * Used for the 'within' spatial filter when combined with element type matching.
 * 
 * @param {WebElement} parentElement - Parent element to search within
 * @param {WebElement[]} candidateElements - Elements to filter
 * @returns {WebElement[]} Elements that are descendants of parent
 */
export function filterWithinParent(parentElement, candidateElements) {
 ...
 * 
 * @param {Object} elemA - Element with rect property
 * @param {Object} elemB - Element with rect property
 * @param {boolean} [applyDirectionalPenalty] - Apply penalty for leftward/upward moves
 * @param {number} [penaltyFactor] - Penalty multiplier (default: 5)
...
export function calculateDistance(elemA, elemB, applyDirectionalPenalty = false, penaltyFactor = 5) {
  ...
  if (applyDirectionalPenalty) {
    // Penalize leftward/upward moves to prefer forward matches
...
/**
 * Sorts elements by distance from a reference element.
 * 
 * @param {WebElement[]} elements - Elements to sort
 * @param {WebElement} reference - Reference element
 * @param {boolean} [applyPenalty] - Whether to apply directional penalties
 * @returns {WebElement[]} Sorted elements (closest first)
 */
export function sortByDistance(elements, reference, applyPenalty = true) {
  return [...elements].sort((a, b) => {
    const distA = calculateDistance(reference, a, applyPenalty);
    const distB = calculateDistance(reference, b, applyPenalty);
    return distA - distB;
  });
}
